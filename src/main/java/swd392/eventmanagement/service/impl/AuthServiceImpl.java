...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public JwtResponse authenticateWithGoogle(GoogleTokenRequest request) {
        Payload payload;
        try {
            payload = googleTokenVerifierService.verifyGoogleIdToken(request.getIdToken());
        } catch (GeneralSecurityException | IOException e) {
            logger.warn("Exception during Google token verification", e);
            throw new InvalidGoogleTokenException("Invalid ID token");
        }
        if (payload == null) {
        ...
        if (!googleTokenVerifierService.isAllowedDomain(email)) {
            ...
            throw new UnauthorizedDomainException("Access denied: Only allowed email domains can access this service");
        }
    ...
    public User createOrUpdateUser(String email, String name, String providerId) {
        ...
        if (userOptional.isEmpty()) {
            ...
            Role userRole = roleRepository.findByName(roleName)
                    .orElseThrow(() -> new UsernameNotFoundException("Error: Role " + roleName + " is not found."));
