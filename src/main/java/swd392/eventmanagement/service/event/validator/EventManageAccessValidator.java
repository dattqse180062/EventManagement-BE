...
public class EventManageAccessValidator {
    ...
    public Department validateUserDepartmentAccess(String departmentCode) {
        ...
        if (!isHeadOfDepartment(department)) {
            throw new AccessDeniedException(
                    "Access denied. Only department HEAD can update events for department: "
                            + departmentCode);
        }
    ...
    /**
     * Validates that an event belongs to the specified department
     * 
     * @param event          The event to validate
     * @param department     The department to check against
     * @param departmentCode Code of the department (used for error messages)
     * @throws AccessDeniedException If the event does not belong to the department
     */
    public void validateEventDepartmentAccess(Event event, Department department, String departmentCode) {
        if (!event.getDepartment().getId().equals(department.getId())) {
            logger.warn("Event with ID: {} does not belong to department with code: {}", event.getId(), departmentCode);
            throw new AccessDeniedException(
                    "Access denied. Event with ID: " + event.getId() + " does not belong to department: "
                            + departmentCode);
        }
    }
